  ...
  useEffect(() => {
    async function loadNearbyPlaces() {
      try {
        ...
        // Fix: "Need Ambulance" and similar always HIGH
        if (
          parsedAnalysis.severity === "LOW" &&
          /ambulance|need help|accident|crash|bleeding|unconscious/i.test(
        ...
        // ✅ Cache real results for offline page
        if (nearbyPlaces.length > 0) {
          const toCache = nearbyPlaces.map((p) => ({
            name:     p.name,
            type:     p.type,
          ...
          localStorage.setItem("roadsos-last-places", JSON.stringify(toCache));
